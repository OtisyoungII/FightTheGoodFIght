...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    // MARK: - Drop Bombs
    @objc func dropBomb() {
        // Drop up to 3 bombs at a time, but not exceeding the width of the paddle
        let bombsToDrop = min(3, bombCount)  // Ensure no more than 3 bombs drop at once
        ...
        for _ in 0..<bombsToDrop {
            ...
            // Drop bombs within the area of the catcher
            let minX = paddle.position.x - paddle.size.width / 2 + newBomb.size.width / 2
            let maxX = paddle.position.x + paddle.size.width / 2 - newBomb.size.width / 2
            newBomb.position = CGPoint(x: CGFloat.random(in: minX..<maxX), y: bossGuy.position.y - 50)
            addChild(newBomb)
